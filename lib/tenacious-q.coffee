...
class TenaciousQ
    ...
    constructor: (@amqpc, @queue, options={}) ->
        ...
        @failureExpiration = (options.failure?.expiration or 60 * 24) * 1000
    ...
    subscribe: (options, listener) =>
        ...
        @exchange.then (ex) =>
            @queue.subscribe options, (msg, headers, info, ack) =>
                @_listen listener, msg, headers, info, new Ack(ex, msg, headers, info, ack, @maxRetries, @failureExpiration)
        .fail (err) ->
            log.error err
